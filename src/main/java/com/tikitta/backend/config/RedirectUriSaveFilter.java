...
public class RedirectUriSaveFilter extends OncePerRequestFilter {
        ...
        if (request.getRequestURI().startsWith("/oauth2/authorization/")) {
            ...
            if (redirectUri != null && !redirectUri.isEmpty()) {
                if (isAuthorizedRedirectUri(redirectUri)) {
                    ...
                    System.out.println("저장 실패: 허용되지 않은 Redirect URI 입니다. (허용된 Base URL: " + authorizedRedirectBaseUrl + ")");
                }
            } else {
    ...
    private boolean isAuthorizedRedirectUri(String uri) {
        // 절대 경로이거나, '/'로 시작하는 상대 경로이면 허용 (임시 조치)
        return uri.startsWith(authorizedRedirectBaseUrl) || uri.startsWith("/");
    }
}